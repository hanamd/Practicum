...
root_dir = 'XXXXX/Documents/Practicum1/practicum1.0/FinalProjectCode/Doc_label-4'
splits = ['train', 'valid']
...
"""---
## Crop + Label-Aware OCR with Table Transformer
...
# Class map from data.yaml from Roboflow labeled data
CLASS_NAMES = {
    0: 'header',
...
# YOLO polygon/bbox → pixel coords ─────────────────────────────────
...
for split in ['train', 'valid']:
    ...
    # Skeep only trainable label types and short text
    df = df[df['label'].isin(KEEP_LABELS)].copy()
    df = df[df['text'].astype(str).str.len() <= MAX_CHARS].copy()

    # drop missing, NaN, or empty text (prevents tokenizer crash)
    df = df.dropna(subset=['text'])
    df = df[df['text'].astype(str).str.strip() != '']
    df = df[df['text'].astype(str) != 'nan']

    # drop very short text (single chars = Tesseract misread)
...
def preprocess(batch):
    #  ensure all text values are clean strings — None/NaN crashes tokenizer
